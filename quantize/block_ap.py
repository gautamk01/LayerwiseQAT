...
def block_ap(
    ...
    if args.layer_ordering == 'sensitivity':
        ...
        try:
            ...
            sensitivity_dataloader = [trainloader[i] for i in range(
                min(args.sensitivity_samples, len(trainloader)))]
            ...
            if len(sensitivity_scores) != len(layers):
                logger.warning(
                    f"Sensitivity scores length mismatch: {len(sensitivity_scores)} vs {len(layers)}")
                raise ValueError("Sensitivity computation failed")

            # Analyze sensitivity patterns
            sensitivity_analysis = analyze_sensitivity_patterns(
                sensitivity_scores)
            logger.info(f"Sensitivity analysis: {sensitivity_analysis}")

            # Get layer training order (most sensitive first)
            layer_indices = rank_layers_by_sensitivity(
                sensitivity_scores, order='descending')

            logger.info(
                f"Layer sensitivity scores: {[f'{s:.4f}' for s in sensitivity_scores.tolist()]}")
            logger.info(f"Layer training order: {layer_indices}")
            logger.info(
                f"Sensitivity computation time: {time.time() - start_time:.2f}s")

        except Exception as e:
            logger.error(f"Sensitivity computation failed: {str(e)}")
            logger.info("Falling back to original layer ordering")
            layer_indices = list(range(len(layers)))
            # Default scores for adaptive LR
            sensitivity_scores = torch.ones(len(layers))
    ...
    for i, block_index in enumerate(layer_indices):
        logger.info(
            f"=== Start quantize block {block_index} (training order: {i+1}/{len(layers)}) ===")
        # step 6.1: replace torch.nn.Linear with QuantLinear for QAT
        normalized_sensitivity = 1.0  # Default value

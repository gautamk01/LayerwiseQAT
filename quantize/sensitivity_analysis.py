...
def compute_layer_sensitivity(model, dataloader, metric='fisher', max_samples=64):
    ...
    """
    print(
        f"Computing sensitivity using {metric} metric with {max_samples} samples...")
...
def compute_fisher_sensitivity(model, dataloader, max_samples=64):
    ...
    for layer_idx, layer in enumerate(layers):
        layer_fisher = sum(param_to_fisher[p].sum(
        ) for p in layer.parameters() if p in param_to_fisher)
...
def compute_hessian_sensitivity(model, dataloader, max_samples=32):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        try:
            ...
            param_to_grad = {p: g for p, g in zip(
                model.parameters(), first_grads)}
                ...
                for param in layer.parameters():
                    if param in param_to_grad:
                        ...
                        z = torch.randint_like(
                            grad, high=2, dtype=grad.dtype) * 2 - 1
                        # Hessian-vector product
                        hvp = torch.autograd.grad(
                            grad, param, grad_outputs=z, retain_graph=True)[0]
                        # Trace contribution
            ...
            print(
                f"Warning: Hessian computation failed for batch {batch_idx}: {e}")
            continue

...
class QuantLinear(nn.Module, TritonModuleMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.register_buffer(
            'qweight',
            torch.zeros((math.ceil(infeatures / (32 // self.bits)),
                        outfeatures), dtype=torch.int32)
        )
        self.register_parameter(
            'scales',
            torch.nn.Parameter(torch.zeros(
                (math.ceil(infeatures / self.group_size), outfeatures), dtype=torch.float16))
        )
        self.register_buffer(
            'qzeros',
            torch.zeros((math.ceil(infeatures / self.group_size),
                        math.ceil(outfeatures / (32 // self.bits))), dtype=torch.int32)
        )
        self.register_buffer(
            'g_idx',
            torch.tensor(
    ...
    def use_fake_quantization(self, del_quant=False, transpose=False):
        ...
        zeros = dequant_dim1(self.qzeros, self.bits,
                             self.maxq, self.zeros_dim0, self.zeros_dim1)
    ...
    def pack(self, linear, scales, zeros, g_idx=None):
        ...
        qzeros = np.zeros((zeros.shape[0], math.ceil(
            zeros.shape[1] / (32 // self.bits))), dtype=np.uint32)
        i = 0
        col = 0
...
def load_quantized_model(model_path, wbits, group_size):
    ...
    # 1. Create an empty model with the quantized architecture
        ...
        for i in range(len(layers)):
            ...
            for name, module in named_linears.items():
                q_linear = QuantLinear(
                    wbits, group_size, module.in_features, module.out_features, not module.bias is None)
                set_op_by_name(layer, name, q_linear)

    model.tie_weights()
    device_map = infer_auto_device_map(model)

    # 2. Load the checkpoint, ignoring the missing 'weight' parameter which is expected
    ...
    load_checkpoint_in_model(
        ...
        ignore_mismatched_sizes=True  # This will ignore the missing 'weight'
    )

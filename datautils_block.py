...
def get_wikitext2(tokenizer, train_size, val_size, seed, seqlen, test_only):
    ...
    random.seed(seed)
    trainloader = []
    # sample train from [0:0.9] and val from [0.9:1.0] to avoid overlap
    ...
    for _ in range(val_size):
        i = random.randint(int(
            trainenc.input_ids.shape[1]*val_sample_ratio) - seqlen - 1, trainenc.input_ids.shape[1] - seqlen - 1)
        j = i + seqlen
        inp = trainenc.input_ids[:, i:j]
        tar = inp.clone()
        tar[:, :-1] = -100
...
def get_c4(tokenizer, train_size, val_size, seed, seqlen, test_only):
    ...
    if test_only:
        ...
        for data in valdata:
            if len(valenc) == 256:
                break
            tmp = tokenizer(data['text'], return_tensors='pt')
    ...
    random.seed(seed)
    trainloader = []
    # Take train_size samples from the beginning of the stream
    ...
    valloader = []
    # Take val_size samples from the validation stream
    for data in valdata:
        if len(valloader) == val_size:
            break
        valenc = tokenizer(data['text'], return_tensors='pt')
        if valenc.input_ids.shape[1] >= seqlen + 1:
...
def get_redpajama(tokenizer, train_size, val_size, seed, seqlen):
    ...
    try:
        ...
        traindata = load_dataset(
            "togethercomputer/RedPajama-Data-1T-Sample", split='train', verification_mode='no_checks')
    random.seed(seed)
...
def test_ppl(model, tokenizer, datasets=['wikitext2'], ppl_seqlen=2048):
    ...
    for dataset in datasets:
        ...
        for i in tqdm(range(nsamples)):
            batch = testenc[:, (i * seqlen): ((i + 1) *
                                              seqlen)].to(model.device)
    ...
    def __init__(self, size, seqlen, hidden_size, batch_size, dtype, cache_path='./cache/block_training_data', off_load_to_disk=False):
        ...
        if self.off_load_to_disk:
            ...
            self.data = torch.zeros(
                (self.size//self.batch_size, self.batch_size, self.seqlen, self.hidden_size), dtype=self.dtype)
